...
;(function($) {
	...
	var Mapplic = function(element) {
		...
		self.loc = {
			more: 'More',
			search: 'Search',
			iconfile: './plugin/main/mapplic/images/icons.svg'
		}
		...
		self.addLocations = function(locations, levelid) {
			$.each(locations, function(index, location) {
				...
				if (location.pin.indexOf('hidden') == -1) {
					...
					if (location.title) $('<span class="content-pin-map position-absolute"><span class="title">' + location.title + '</span></span>').appendTo(pin);
